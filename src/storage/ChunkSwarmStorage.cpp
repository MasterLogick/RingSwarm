...
core::Node *getNearestChunkNode(
    ...
    for (const auto &item: chunkSwarmStorage) {
        if (dist(item.first.second, chunkIndex) <=
            dist(min.first.second, chunkIndex)) {
            min = item;
        }
    }
    ...
    Statement nearestChunkNodeSelectStatement(
        dbConnection,
        nearestChunkNodeSelect
    );
...
void storeChunkSwarm(core::ChunkSwarm *chunkSwarm) {
    if (chunkSwarmStorage.contains(
            std::pair(chunkSwarm->link->keyId, chunkSwarm->link->chunkIndex)
        )) {
        if (chunkSwarmStorage[std::pair(
                chunkSwarm->link->keyId,
                chunkSwarm->link->chunkIndex
            )] != chunkSwarm) {
