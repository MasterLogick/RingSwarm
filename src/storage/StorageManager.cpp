...
const char *tableCreateStatement =
    ...
    ");\n"
    "\n"
    "create table if not exists key_swarm\n"
    "(\n"
    "    key_id      blob,\n"
    "    swarm_index integer,\n"
    "    node_id     blob\n"
...
void loadStorage(const char *dbPath) {
    ...
    if ((err = sqlite3_open(dbPath, &dbConnection)) != SQLITE_OK) {
        BOOST_LOG_TRIVIAL(error)
            << "SQLITE3 db open error: " << sqlite3_errstr(err);
        throw StorageException();
    }

    if ((err = sqlite3_trace_v2(
             dbConnection,
             SQLITE_TRACE_STMT,
             [](auto mask, void *ctx, void *b, void *c) {
                 if (mask == SQLITE_TRACE_STMT) {
                     ...
                     BOOST_LOG_TRIVIAL(trace) << "SQLITE3 trace stmt: " << str;
                 }
                 return 0;
             },
             nullptr
        ...
        BOOST_LOG_TRIVIAL(error)
            << "SQLITE3 add sql tracer error: " << sqlite3_errstr(err);
        throw StorageException();
    }
    if ((err = sqlite3_exec(
             dbConnection,
        ...
        BOOST_LOG_TRIVIAL(error)
            << "SQLITE3 table create error: " << sqlite3_errstr(err);
        throw StorageException();
    }
}
