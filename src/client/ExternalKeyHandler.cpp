...
ExternalKeyHandler::ExternalKeyHandler(
    ...
    auto keySwarm =
        std::get<0>(possibleKeySwarmNode->getKeySwarm(keyId)->await());
    ...
    auto zeroChunkSwarm =
        std::get<0>(keySwarmNode->getChunkSwarm(keyId, 0)->await());
    zeroChunkNode =
        std::get<0>(core::getOrConnectToOne(zeroChunkSwarm)->await());

    zeroChunk = std::get<0>(zeroChunkNode->getChunkLink(keyId, 0)->await());
    auto readSize =
        std::get<0>(zeroChunkNode
                        ->getChunk(keyId, 0, 0, &keyInfo, sizeof(core::KeyInfo))
                        ->await());
...
ExternalKeyHandler::readData(uint32_t len, uint64_t offset) {
    ...
    if (iter != data.end()) {
        ...
        zeroChunkNode
            ->getChunk(
                keyId,
                0,
                (part << CACHE_SIZE) + sizeof(core::KeyInfo),
                ptr,
                1 << CACHE_SIZE
            )
