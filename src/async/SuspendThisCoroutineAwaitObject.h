...
class SuspendThisCoroutineAwaitObject {
    std::function<
        std::coroutine_handle<>(std::coroutine_handle<Promise<RetTypes...>>)>
        handler;

public:
    explicit SuspendThisCoroutineAwaitObject(
        std::function<std::coroutine_handle<>(std::coroutine_handle<
                                              Promise<RetTypes...>>)> handler
    )
