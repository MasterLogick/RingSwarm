...
namespace RingSwarm::proto {
    ...
    TransportClientSideWrapper::sendShortRequest(uint8_t commandIndex, RequestBuffer &req, uint32_t maxResponseSize) {
        return sendRequest(commandIndex, req, maxResponseSize)->
                then<std::shared_ptr<transport::Buffer>>([this](ResponseHeader h) {
            auto *buff = new uint8_t[h.responseLen];
            return rawRead(buff, h.responseLen)->
                    then<std::shared_ptr<transport::Buffer>>([buff, len = h.responseLen]() {
