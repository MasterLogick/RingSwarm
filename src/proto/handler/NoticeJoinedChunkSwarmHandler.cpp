...
ClientHandler::noticeJoinedChunkSwarm(core::Id *keyId, uint64_t chunkIndex) {
    RequestBuffer req(40);
    req.write(keyId);
    req.write<uint64_t>(chunkIndex);
    return transport->sendSmallRequest(7, req, MAX_RESPONSE_SIZE)
        ->then<std::vector<core::Node *>>([](auto resp) {
            return resp->template readVec<core::Node *>();
        });
}

void ServerHandler::handleNoticeJoinedChunkSwarm(
