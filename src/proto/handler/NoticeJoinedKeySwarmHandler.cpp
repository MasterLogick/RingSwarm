...
std::shared_ptr<async::Future<std::vector<core::Node *>>>
ClientHandler::noticeJoinedKeySwarm(core::Id *keyId, uint8_t index) {
    RequestBuffer req(33);
    req.write(keyId);
    req.write<uint8_t>(index);
    return transport
        ->sendSmallRequest(sizeof(ResponseHeader), req, MAX_RESPONSE_SIZE)
        ->then<std::vector<core::Node *>>([](auto resp) {
            return resp->template readVec<core::Node *>();
        });
}

void ServerHandler::handleNoticeJoinedKeySwarm(
    ...
    if (keySwarm == nullptr) {
        ...
        if (std::none_of(swarm.begin(), swarm.end(), [this](auto pair) -> bool {
                return *pair.second == *remote;
            })) {
            swarm[index] = remote;
        }
        // todo fix
