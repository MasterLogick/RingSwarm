...
int NodeContext::addServer(std::string host, int port) {
    ...
    return server->listen(
        [this](std::unique_ptr<transport::PlainSocketTransport> serverSideSocket
        ) {
            serverHandlers.push_back(
                handleServerConnection(std::move(serverSideSocket))
            );
        }
    );
}
...
async::Coroutine<> NodeContext::handleServerConnection(
    ...
    auto [a, b] = co_await transport::SecureOverlayTransport::createServerSide(
